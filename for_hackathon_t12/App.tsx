...
// Backend API Client (NEW - replaces old agents)
import { 
  analyzeCoin, 
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (intent.type === 'ANALYZE' && intent.coinName) {
        ...
        // Update portfolio with real-time prices first
        const updatedPortfolio = await updatePortfolioRealTime(userProfile.portfolio);
        const portfolioResult: PortfolioAnalysisResult = await analyzePortfolio(updatedPortfolio);
        setLoadingStatus('');
        ...
        const responseText = await chatWithModel(currentInput, 'web-user', contextData);
